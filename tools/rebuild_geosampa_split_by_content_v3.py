...
class Sample:
    ...
    rel_image: str
    rel_label: str
    bg_kind: str  # positive | drop_natural | easy_natural | hard_urban
    score: dict


def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Rebuild YOLO segmentation train/val split using stricter content filtering with drop_natural support."
    )
    ...
    ap.add_argument("--max-easy-backgrounds-in-val", type=int, default=8)
    ap.add_argument("--train-easy-ratio", type=float, default=0.20)

    ap.add_argument("--drop-natural-from-train", action="store_true", default=True)
    ap.add_argument("--drop-natural-from-val", action="store_true", default=True)

    ap.add_argument("--downsample-max-side", type=int, default=256)
    ap.add_argument("--easy-threshold", type=float, default=0.62)

    ap.add_argument("--water-drop-blue-ratio", type=float, default=0.35)
    ap.add_argument("--water-drop-line-score-max", type=float, default=0.03)
    ap.add_argument("--water-drop-edge-density-max", type=float, default=0.06)

    ap.add_argument("--forest-drop-green-ratio", type=float, default=0.45)
    ap.add_argument("--forest-drop-line-score-max", type=float, default=0.03)
    ap.add_argument("--forest-drop-edge-density-max", type=float, default=0.05)

    ap.add_argument("--drop-natural-score-min", type=float, default=0.72)
    ap.add_argument("--drop-urban-score-max", type=float, default=0.18)

    ap.add_argument("--min-blue-ratio-for-water-bias", type=float, default=0.08)
    ap.add_argument("--min-green-ratio-for-vegetation-bias", type=float, default=0.16)
    ap.add_argument("--edge-density-hard-threshold", type=float, default=0.055)
    ap.add_argument("--line-score-hard-threshold", type=float, default=0.08)
...
def classify_background(score: dict, args: argparse.Namespace) -> str:
    ...
    urban_hard_score = float(score["urban_hard_score"])

    if (
        blue_ratio >= args.water_drop_blue_ratio
        and line_score <= args.water_drop_line_score_max
        and edge_density <= args.water_drop_edge_density_max
    ):
        return "drop_natural"

    if (
        green_ratio >= args.forest_drop_green_ratio
        and line_score <= args.forest_drop_line_score_max
        and edge_density <= args.forest_drop_edge_density_max
    ):
        return "drop_natural"

    if (
        natural_easy_score >= args.drop_natural_score_min
        and urban_hard_score <= args.drop_urban_score_max
    ):
        return "drop_natural"

    if edge_density >= args.edge_density_hard_threshold or line_score >= args.line_score_hard_threshold:
        return "hard_urban"

    if blue_ratio >= args.min_blue_ratio_for_water_bias and natural_easy_score >= (args.easy_threshold - 0.05):
        return "easy_natural"

    if green_ratio >= args.min_green_ratio_for_vegetation_bias and natural_easy_score >= (args.easy_threshold - 0.03):
        return "easy_natural"

    if natural_easy_score >= args.easy_threshold:
        return "easy_natural"

    return "hard_urban"


def load_split_samples(dataset_root: Path, split: str, args: argparse.Namespace) -> list[Sample]:
    ...
    for label_path in sorted(lab_dir.glob("*.txt")):
        ...
        if is_positive:
            ...
            bg_kind = classify_background(score, args)
...
def summarize(samples: list[Sample]) -> dict[str, int]:
    return {
        ...
        "drop_natural": sum(1 for s in samples if s.bg_kind == "drop_natural"),
...
def main() -> int:
    ...
    drop_bgs = [s for s in all_samples if s.bg_kind == "drop_natural"]
    ...
    val_samples = positives[:val_pos_n] + hard_bgs[:val_hard_n] + easy_bgs[:val_easy_n]

    if not args.drop_natural_from_val:
        remaining_slots = args.val_backgrounds - (val_easy_n + val_hard_n)
        if remaining_slots > 0:
            val_samples += drop_bgs[:remaining_slots]
    ...
    train_drop = [s for s in remaining_samples if s.bg_kind == "drop_natural"]
    ...
    if args.drop_natural_from_train:
        kept_train_drop = []
        dropped_train_drop = train_drop
    else:
        kept_train_drop = train_drop
        dropped_train_drop = []

    train_samples = train_pos + train_hard + kept_train_easy + kept_train_drop
    ...
    summary = {
        ...
        "params": {
            ...
            "drop_natural_from_train": args.drop_natural_from_train,
            "drop_natural_from_val": args.drop_natural_from_val,
            "downsample_max_side": args.downsample_max_side,
            "easy_threshold": args.easy_threshold,
            "water_drop_blue_ratio": args.water_drop_blue_ratio,
            "water_drop_line_score_max": args.water_drop_line_score_max,
            "water_drop_edge_density_max": args.water_drop_edge_density_max,
            "forest_drop_green_ratio": args.forest_drop_green_ratio,
            "forest_drop_line_score_max": args.forest_drop_line_score_max,
            "forest_drop_edge_density_max": args.forest_drop_edge_density_max,
            "drop_natural_score_min": args.drop_natural_score_min,
            "drop_urban_score_max": args.drop_urban_score_max,
        },
        ...
        "dropped": {
            ...
            "dropped_train_drop_natural": len(dropped_train_drop),
        },
        "preview": {
            "val_easy_natural": [
                {
                    ...
                    "line_score": s.score.get("line_score"),
                    "edge_density": s.score.get("edge_density"),
                }
                for s in val_samples if s.bg_kind == "easy_natural"
            ][:30],
            "val_hard_urban": [
                {
                    "rel_image": s.rel_image,
                ...
                for s in val_samples if s.bg_kind == "hard_urban"
            ][:30],
            "dropped_train_drop_natural": [
                {
                    "rel_image": s.rel_image,
                    "natural_easy_score": s.score.get("natural_easy_score"),
                    "urban_hard_score": s.score.get("urban_hard_score"),
                ...
                for s in dropped_train_drop[:50]
            ],
        },
    }
    ...
    print(
        f"Source drop/easy/hard backgrounds: "
        f"{src_summary['drop_natural']}/{src_summary['easy_natural']}/{src_summary['hard_urban']}"
    )
    print()
    print(
        ...
        f"drop_natural={train_summary['drop_natural']} | "
    ...
    print(
        ...
        f"drop_natural={val_summary['drop_natural']} | "
    ...
    print(f"Dropped drop_natural from train: {len(dropped_train_drop)}")
